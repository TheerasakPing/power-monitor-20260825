    ...
    if($_POST["mode"] == "ENERGY"){
        if($phase == 2){
            $sql = "SELECT
                    ...
                    FROM `tb_data_sensor_mini` WHERE `data_sn`= ? 
                        AND `data_e` != 0 
                        AND `data_v` != 0
                        AND SUBSTRING(`data_timestamp`,1,10) BETWEEN ? AND ? group by SUBSTRING(`data_timestamp`,1,15) ORDER BY `data_timestamp`"; // hour(SUBSTRING(`data_timestamp`,14,5)), day(SUBSTRING(`data_timestamp`,1,10)) ORDER BY `tb_data_sensor_mini`.`data_timestamp`";
        }else{ // emter big
            $sql = "SELECT
                        -- SUBSTRING(`data_timestamp`,1,15) AS timestamp,
                        UNIX_TIMESTAMP(STR_TO_DATE(SUBSTRING(data_timestamp,1,15), '%Y/%m/%d - %H')) AS  ts,
                        round(MAX(`data_e`) - MIN(`data_e`), 2) AS energy
                    FROM `tb_data_sensor` WHERE `data_sn`= ? 
                        AND `data_e` != 0 
                        AND `data_v_L-N-AVG` != 0 
                        AND SUBSTRING(`data_timestamp`,1,18) BETWEEN ? AND ? group by SUBSTRING(`data_timestamp`,1,15) ORDER BY `data_timestamp`";// hour(SUBSTRING(`data_timestamp`,14,5)), day(SUBSTRING(`data_timestamp`,1,10)) ORDER BY `tb_data_sensor`.`data_timestamp`";
        }
        ...
        if ($phase == 3) {
            $sql = "SELECT
                ...
                FROM `tb_data_sensor` WHERE `data_sn`= ? 
                    AND `data_e` != 0 
                    AND `data_v_L-N-AVG` != 0 
                    AND SUBSTRING(`data_timestamp`,1,18) BETWEEN ? AND ? group by SUBSTRING(`data_timestamp`,1,18) ORDER BY `data_timestamp`";
        }
        elseif($phase == 2){
            $sql = "SELECT
                -- `data_timestamp`,
