...
try {
    ...
    if (!$limit['allowed']) {
        ...
        json_error('Too many login attempts. Please try again later.', 429, ['retry_after'=>$limit['retry_after']]);
    }
} catch (Throwable $e) {
...
try {
    if (!verify_turnstile($turnstileToken, $ip)) json_error('Human verification failed. Please try again.', 403);
} catch (Throwable $e) {
...
if (!$row_count) {
    echo json_encode(['status'=>false,'message'=>'No user'], JSON_UNESCAPED_UNICODE);
    exit();
}
...
if (!empty($hashInfo['algo'])) {
    ...
    $passwordIsLegacy = $passwordValid;
}
if (!$passwordValid) {
    echo json_encode(['status'=>false,'message'=>'No user'], JSON_UNESCAPED_UNICODE);
    exit();
}
if ($passwordIsLegacy || password_needs_rehash($storedPassword, PASSWORD_DEFAULT)) {
    $rehash = password_hash($password, PASSWORD_DEFAULT);
    $rehashStmt = $dbcon->prepare('UPDATE tb_account SET account_pass = :pass WHERE account_id = :id');
    $rehashStmt->execute([':pass'=>$rehash, ':id'=>$row_count['account_id']]);
}

if ((int)$row_count['account_suspend'] === 0) {
    echo json_encode(['status'=>false,'message'=>'Suspend'], JSON_UNESCAPED_UNICODE);
    exit();
}
...
$level_1 = (int)$dbcon->query("SELECT COUNT(userST_level) FROM tb_user_st WHERE userST_accountID=" . (int)$u_id . " AND userST_level=1")->fetchColumn();
if ($level_1 > 0) {
    $level = 1;
    ...
    $level_2 = (int)$dbcon->query("SELECT COUNT(userST_level) FROM tb_user_st WHERE userST_accountID=" . (int)$u_id . " AND userST_level=2")->fetchColumn();
    $level = $level_2 > 0 ? 2 : 3;
    $count_site = (int)$dbcon->query("SELECT userST_siteID FROM tb_user_st WHERE userST_accountID=" . (int)$u_id . " GROUP BY userST_siteID")->rowCount();
    $stmt = $dbcon->query("SELECT * FROM tb_user_st INNER JOIN tb_house ON tb_user_st.userST_houseID=tb_house.house_id INNER JOIN tb_site ON tb_user_st.userST_siteID=tb_site.site_id WHERE userST_accountID=" . (int)$u_id . " GROUP BY userST_houseID");
    ...
    if ($count_site === 1 && $count_House === 1) {
        ...
        $display = ['countSite'=>$count_site,'select'=>['siteID'=>'','houseID'=>'','sn'=>'','phase'=>'','temp'=>'']];
    }
    $stmtSH = $dbcon->query("SELECT * FROM tb_user_st INNER JOIN tb_site ON tb_user_st.userST_siteID=tb_site.site_id INNER JOIN tb_house ON tb_user_st.userST_houseID=tb_house.house_id WHERE userST_accountID=" . (int)$u_id);
}
...
echo json_encode([
    ...
    'data'=>[
        'account'=>['id'=>$u_id,'user'=>$account_user,'img'=>$account_img,'level'=>$level],
        'theme'=>$account_theme,
        'display'=>$display,
        'dt'=>date('Y-m-d H:i:s')
    ]
